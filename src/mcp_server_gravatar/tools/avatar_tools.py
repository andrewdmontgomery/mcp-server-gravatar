...
async def _get_avatars(selected_email_hash: str | None = None) -> list[dict[str, Any]]:
    """
...
async def _get_selected_avatar_as_image(email: str | None = None) -> list[bytes]:
    """
    ...
    avatars = await _get_avatars(selected_email_hash=selected_hash)
    images: list[bytes] = []
...
def register_tools(mcp: FastMCP):
    ...
    async def get_avatars(selected_email_hash: str | None = None) -> list[dict[str, Any]]:
        ...
        Returns:
            list[dict[str, Any]]: A list of avatar metadata dictionaries.
        """
        return await _get_avatars(selected_email_hash)
    ...
    async def get_selected_avatar_as_image(email: str | None = None) -> list[bytes]:
        ...
        Returns:
            list[bytes]: A single-element list containing the selected avatar image bytes.
        """
        return await _get_selected_avatar_as_image(email)
