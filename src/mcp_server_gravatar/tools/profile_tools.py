...
async def _get_profile_by_email(email: str) -> dict[str, Any]:
    """
...
async def _get_profile_by_hash(hash: str) -> dict[str, Any]:
    """
...
@overload
async def _get_profile_field_with_hash(
...
async def _get_profile_field_with_hash(
    ...
    """
    profile = await _get_profile_by_hash(profileIdentifier)
...
@overload
async def _get_profile_field_with_email(
    email: str,
    field: Literal[
        "hash", "display_name", "profile_url", "avatar_url", "avatar_alt_text",
        "location", "description", "job_title", "company",
...
async def _get_profile_field_with_email(
    ...
    return await _get_profile_field_with_hash(profile_hash, field)
...
def register_tools(mcp: FastMCP):
    ...
    async def get_profile_by_email(email: str) -> dict[str, Any]:
        ...
        Returns:
            dict[str, Any]: The JSON-deserialized Gravatar profile object.
        """
        return await _get_profile_by_email(email)
    ...
    async def get_profile_by_hash(hash: str) -> dict[str, Any]:
        ...
        Returns:
            dict[str, Any]: The JSON-deserialized Gravatar profile object.
        """
        return await _get_profile_by_hash(hash)
    ...
    async def get_profile_field_with_hash(
        ...
        Returns a type-safe union based on the requested field.
        """
        return await _get_profile_field_with_hash(profileIdentifier=profileIdentifier, field=field)
    ...
    async def get_profile_field_with_email(
        ...
        Fetch a specific field from a Gravatar profile by email address.
        """
        return await _get_profile_field_with_email(email=email, field=field)
